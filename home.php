<?php
/**
 * Blog index (v2) — sirve cuando WP usa esta página como Page for Posts.
 * Mismo render que template-blog-v2.php pero usando el WP Loop principal.
 *
 * @package Morillo
 */
get_header();
			...
			<?php if ( have_posts() ) : ?>
				<div class="v2-blog-grid v2-blog-page__grid" data-stagger>
					<?php while ( have_posts() ) : the_post();
						...
						$min    = max( 1, ceil( $words / 200 ) );
					?>
						<a class="v2-blog-card" href="<?php the_permalink(); ?>">
							<p class="v2-blog-card__meta">[<?php echo esc_html( $cat ); ?> · <?php echo esc_html( strtoupper( get_the_date( 'd M Y' ) ) ); ?> · <?php echo esc_html( $min ); ?> MIN]</p>
